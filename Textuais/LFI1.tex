...
Este capítulo é dividido da seguinte forma: na Seção~\ref{sec:linguagem} é apresentada a linguagem da lógica proposicional \lfium{} bem como definições necessárias para desenvolver as provas de metateoremas. A Seção~\ref{sec:axiomatizacao} contém uma breve explicação sobre sistemas de prova sintáticos e uma Axiomatização de Hilbert para a \lfium{} é definida. Na Seção~\ref{sec:semantica} a semântica da \lfium{} é definida a partir de matrizes lógicas e de uma semântica de valorações não determinística\migscortar{. Ainda na Seção~\ref{sec:semantica}, as matrizes e a semântica de valorações são provadas equivalentes. \helena{Não sei como NÃO separar estas últimas duas frases.}}{, assim como é provada a equivalência entre essas duas semânticas.}
...
\label{sec:axiomatizacao}
    ...
    \begin{definicao}[Derivação em \lfium{}]
        Seja $\Gamma \cup \{\phi\} \subseteq \pazocal{L}_{\Sigma}$ um conjunto de fórmulas, uma derivação $\Gamma \vdash \phi$ de $\phi$ a partir de $\Gamma$ em \lfium{} é uma sequência finita de fórmulas \(\phi_0, \dots, \phi_n\) onde, para cada $1 \leqslant i \leqslant n$, \migscortar{é válido que}{ alguma dessas condições é satisfeita (não gostei nem de como tava antes nem de como ficou minha modificação, mas acho que o que estava antes não está muito bom)}:
        \begin{align*}
            & \text{~~(i) }\phi_{i} \text{ é instância de substituição de um axioma;}\\
            & \text{~(ii) }\phi_{i} \in \Gamma;\\
            & \text{(iii) }\text{existem } j,k \leqslant i \text{ de modo que } \phi_{i} \text{ é o resultado da aplicação de MP em } \phi_{j} \text{ e } \phi_{k}.\tag*\qed{}
        \end{align*}
    \end{definicao}
...
\section{Metateoremas}
    ...
    \noindent Antes de apresentar a prova do Teorema~\ref{teo:deducao} é conveniente provar o seguinte lema auxiliar. \migs{Já que você vai apresentar esse lema antes de apresentar a prova do teorema, enuncie e prove ele antes do teorema e então vá para o teorema.}
    ...
    \begin{proof}[Prova do Lema~\ref{lem:id}]
        A seguinte sequência de derivação demonstra o lema: \migs{Acho interessante você explicitar quais variáveis foram substituídas por o que quando lista qual axioma foi utilizado, igual faria no quadro branco.}
    ...
    \begin{proof}[Prova do Teorema~\ref{teo:deducao}] A prova será dividida em duas partes:\\
        ...
        \noindent 1. Caso $\phi_{1}$ seja instância de substituição de um axioma, a seguinte dedução mostra $\Gamma \conhil \alpha \to \phi_{1}$:
        \begin{align*}
            & \text{1. } \phi_{1} \tag{Suposição}\\
            & \text{2. } \phi_{1} \to (\alpha \to \phi_{1}) \tag{Ax1}\\
            & \text{3. } \alpha \to \phi_{1} \tag{MP 1,2}
        \end{align*}
        
        \noindent 2. Caso $\phi_{1} \in \Gamma \cup \{\alpha\}$, existem dois casos a serem considerados:
        \begin{enumerate}
            \item $\phi_{1} = \alpha$
            \item $\phi_{1} \in \Gamma$
        \end{enumerate}

        \noindent 2.1. Caso $\phi_{1} = \alpha$, é necessário mostrar $\Gamma \conhil \alpha \to \alpha$, que é o que foi provado pelo Lema~\ref{lem:id}.

        \noindent 2.2. Caso $\phi_{1} \in \Gamma$, então $\Gamma \conhil \alpha \to \phi_{1}$ é provado pela \migscortar{seguinte sequência de derivações:}{ mesma sequência de derivações que foi apresentada no caso de $\phi_1$ ser uma instância de um axioma.}
        \begin{align*}
        ...
        \noindent \underline{Passo} Hipótese de indução (HI): \textit{Para qualquer sequência de dedução $\phi_{1} \ldots \phi_{n}$ de $\Gamma, \alpha \conhil \beta$ tem-se $\Gamma \conhil \alpha \to \beta$}. É preciso mostrar que $\Gamma \conhil \alpha \to \beta$ segue caso a dedução seja de tamanho $n + 1$. Ao analisar a obtenção de $\phi_{n + 1} = \beta$ na sequência de dedução de $\Gamma, \alpha\conhil \beta$, existem três casos a se considerar:
        \begin{enumerate}
            \item $\phi_{n + 1}$ é instância de substituição de um axioma.
            \item $\phi_{n + 1} \in \Gamma \cup \{\alpha\}$.
            \item $\phi_{n + 1}$ é obtido por \textit{modus ponens} em duas fórmulas $\phi_{j}$ e $\phi_{k}$ com $j, k \leqslant n + 1$.
        \end{enumerate}
        
        \noindent Os casos 1 e 2 são análogos aos casos provados na base.

        \noindent 3. Caso $\phi_{n + 1}$ seja obtido por \textit{modus ponens} em duas fórmulas $\phi_{j}$ e $\phi_{k}$ com $j, k \leqslant n + 1$. Então $\phi_{k} = \phi_{j} \to \phi_{n + 1}$ (ou $\phi_{j} = \phi_{k} \to \phi_{n + 1}$, a prova para este caso é análoga). Como $j, k \leqslant n + 1$, pela hipótese de indução segue que $\Gamma \conhil \alpha \to \phi_{j}$ e $\Gamma \conhil \alpha \to \phi_{k}$. A seguinte sequência de dedução mostra $\Gamma \conhil \alpha \to \phi_{n + 1}$:
        \begin{align*}
            & \text{1. } \alpha \to \phi_{j} \tag{HI sobre $\phi_{j}$}\\
            & \text{2. } \alpha \to (\phi_{j} \to \phi_{n + 1}) \tag{HI sobre $\phi_{k}$}\\
            & \text{3. } (\alpha \to (\phi_{j} \to \phi_{n + 1})) \to ((\alpha \to \phi_{j}) \to (\alpha \to \phi_{n + 1})) \tag{Ax2}\\
            & \text{4. } (\alpha \to \phi_{j}) \to (\alpha \to \phi_{n + 1}) \tag{MP 2,3}\\
            & \text{5. } \alpha \to \phi_{n + 1} \tag{MP 1,4}
        \end{align*}

        Com isto, a prova de $\Gamma, \alpha \conhil \beta \Longleftrightarrow \Gamma \conhil \alpha \to \beta$ foi demonstrada. \migs{Esse comentário é desnecessário.}
    \end{proof}
