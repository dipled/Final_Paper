...
\section{Metateoremas}\label{sec:metateoremas}
    ...
    \begin{lema}[Lindenbaum-{\L}os]\label{lem:lindenbaum}
        Para qualquer lógica tarskiana e finitária $\mathcal{L} = \langle \pazocal{L}, \Vdash \rangle$, para qualquer conjunto de fórmulas $\Gamma \cup \{\phi\} \subseteq \pazocal{L}$, se $\Gamma \nVdash \phi$ então existe um conjunto de fórmulas $\Delta$, com $\Gamma \subseteq \Delta \subseteq \pazocal{L}$, tal que $\Delta$ é um conjunto maximal não-trivial em relação a $\phi$ em $\mathcal{L}$.
    \end{lema}

    \begin{proof}[Prova do Lema~\ref{lem:lindenbaum}]\setbool{@fleqn}{false}
        Seja $\mathcal{L} = \langle \pazocal{L}, \Vdash \rangle$ uma lógica tarskiana e finitária e seja $\Gamma \cup \{\phi\} \subseteq \pazocal{L}$ um conjunto de fórmulas com $\Gamma \nVdash \phi$. Arranje as fórmulas de $\pazocal{L}$ numa sequência $\phi_1 \ldots \phi_i \ldots$ e defina $\Gamma_i$ recursivamente da seguinte forma:
        \begin{align*}
                \text{(i)}~&\Gamma_0 = \Gamma\\
                \text{(ii)}~&\Gamma_i =
                \begin{cases}
                    \Gamma_{i - 1} &\text{ se } \Gamma_{i - 1}, \phi_i \Vdash \phi\\
                    \Gamma_{i - 1} \cup \{\phi_i\} &\text{ se } \Gamma_{i - 1}, \phi_i \nVdash \phi
                \end{cases}
        \end{align*}

        Então, defina um conjunto de fórmulas $\Delta = \bigcup_{i=0}^{\infty}\Gamma_i$. Dividiremos a prova em algumas condições a serem provadas:
        
            (1) $\Gamma \subseteq \Delta \subseteq \pazocal{L}$.
            \begin{adjustwidth}{2cm}{}
                
                Pela construção de $\Gamma_i$, temos $\Gamma_0 = \Gamma \subseteq \Gamma_i$ e $\Gamma_i \subseteq \pazocal{L}$ para todo $i \in \mathbb{N}$. Como $\Delta$ é formado a partir da união de todos os conjuntos $\Gamma_i$, temos $\Gamma_0 \subseteq \Delta$ e $\Delta \subseteq \pazocal{L}$.
            \end{adjustwidth}
            
            (2) $\Gamma_i \nVdash \phi$ para todo $i \geq 0$.
            \begin{adjustwidth}{2cm}{}Provaremos por indução:

                \textbf{\textsc{Base}} $i = 0$.
    
                Temos $\Gamma_0 \nVdash \phi$ pela nossa hipótese de $\Gamma \nVdash \phi$.
    
                \textbf{\textsc{Passo}} Hipótese de indução (HI): Para qualquer $k < i$, temos $\Gamma_k \nVdash \phi$.
    
                 Logo temos $\Gamma_{i-1} \nVdash \phi$ por (HI). Portanto, pela definição de $\Gamma_i$, temos $\Gamma_i \nVdash \phi$.
            ...
            (4) $\Delta$ é maximal não-trivial em relação a $\phi$ em $\mathcal{L}$.
