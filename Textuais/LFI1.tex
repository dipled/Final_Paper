...
\section{Metateoremas}\label{sec:metateoremas}
    ...
    \begin{proof}[Prova da Proposição~\ref{prop:tarski}]
        ...
        \end{itemize}
        \begin{adjustwidth}{1cm}{}
            \textbf{Prova do item (i)}. Vamos supor $\phi \in \Gamma$. Então \migscortar{a seguinte derivação mostra $\Gamma \conhil \phi$:}{, por definição, temos que $\Gamma \conhil \phi$.}
            % \begin{align*}
            %     1.~& \phi\tag{Premissa}
            % \end{align*}

            \noindent{}\textbf{Prova do item (ii)}. Vamos supor $\Delta \conhil \phi$ e $\Delta \subseteq \Gamma$. Então, existe uma prova (sequência de fórmulas) de $\phi$ a partir do conjunto de fórmulas $\Delta$. \migs{Como, pela suposição, temos que essa sequência está inteiramente contida em $\Gamma$ concluímos que} a mesma sequência de fórmulas prova $\Gamma \conhil \phi$.
