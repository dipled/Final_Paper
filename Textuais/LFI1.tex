...
\section{Metateoremas}\label{sec:metateoremas}
    ...
    \begin{proof}[Prova do Lema~\ref{lem:matval}]
        ...
        \begin{provaporcasos}
            ...
            \helena{ok, mas antes de eu fazer isso, será que precisa mudar o layout desse prova? tipo, eu acho que tá ok de entender e não tá TÃO feio, mas a kaqui tinha reclamado}
            ...
            \begin{provaporsubcasos}
                \subcasodeprova{$h(\phi) = h(\alpha \land \beta) = h(\alpha) \land h(\beta) = 1$ sse $v(\phi) = v(\alpha \land \beta) = 1$ e $v(\neg \phi) = v(\neg (\alpha \land \beta)) = 0$.}
                    ...
                    $\Longleftrightarrow$ $h(\alpha) = 1$ e {$h(\beta) = 1$} \hfill (Matrix de $\land$)

                    $\Longleftrightarrow$ $v(\alpha) = 1$, $v(\neg \alpha) = 0$, $v(\beta) = 1$ e $v(\neg \beta) = 0$ \hfill (HI)

                    $\Longleftrightarrow$ $v(\phi) = v(\alpha \land \beta) = 1$ \hfill $(vAnd)$
                    
                    $\Longleftrightarrow$ $v(\neg \phi) = v(\neg(\alpha \land \beta)) = 0$ \hfill $(vDM_{\land})$
                ...
                \subcasodeprova{$h(\phi) = h(\alpha \land \beta) = h(\alpha) \land h(\beta) = \meio{}$ sse $v(\phi) = v(\alpha \land \beta) = 1$ e $v(\neg \phi) = v(\neg (\alpha \land \beta)) = 1$.}
                    ...
                    $\Longleftrightarrow$ ($h(\alpha), h(\beta) \in \{1, \meio{}\}$) e ($h(\alpha) = \meio{}$ ou $h(\beta) = \meio{}$) \hfill (Matriz de $\land$)
                    
                    $\Longleftrightarrow$, ($v(\alpha) = 1$ e $v(\beta) = 1$) e ($v(\alpha) = v(\neg \alpha) = 1$ ou $v(\beta) = v(\neg \beta) = 1$) \hfill (HI)

                    $\Longleftrightarrow$, ($v(\alpha) = 1$, $v(\neg \alpha) = 1$ e $v(\beta) = 1$) ou ($v(\alpha) = 1$, $v(\neg \beta) = 1$ e $v(\beta) = 1$) \hfill (Dist. $\land, \lor$ e Idemp. $\land$)

                    $\Longleftrightarrow$ ($v(\alpha) = 1$ e $v(\beta) = 1$) e ($v(\neg \alpha) = 1$ ou $v(\neg \beta) = 1$) \hfill (Dist. $\land, \lor$ e Idemp. $\land$)

                    $\Longleftrightarrow$ ($v(\phi) = v(\alpha \land \beta) = 1$) e ($v(\neg \phi) = v(\neg (\alpha \land \beta)) = 1$) \hfill $(vAnd)$ e $(vDM_{\land})$
            ...
            \begin{provaporsubcasos}
                ...
                \subcasodeprova{$h(\phi) = h(\alpha \lor \beta) = h(\alpha) \lor h(\beta) = \meio{}$ sse $v(\phi) = v(\alpha \lor \beta) = 1$ e $v(\neg \phi) = v(\neg (\alpha \lor \beta)) = 1$.}
    ...
    \begin{proof}[Prova do Lema~\ref{lem:lindenbaum}]\setbool{@fleqn}{false}
        Dada $\lfium{} = \langle \ling, \conhil \rangle$ e seja $\Gamma \cup \{\phi\} \subseteq \ling$ um conjunto de fórmulas com $\Gamma \nconhil \phi$. Arranje \migs{Como?} \helena{TAMBÉM NÃO SEI} \migs{Como não sabe mulher, foi tu quem fez a prova!?} \helena{é que eu não parei pra pensar se eu preciso dizer como arranjar as fórmulas. Tipo, existe um jeito de arranjar and that's it} \migs{Você poderia no mínimo colocar em uma nota de rodapé tipo ``podemos ordenar as fórmulas em ordem lexicográfica blah blah blah''} as fórmulas de $\ling$ numa sequência $\pazocal{C} = \phi_1, \phi_2, \ldots, \phi_i, \ldots$ e defina $\Gamma_i$ recursivamente da seguinte forma:
        \begin{align*}
    ...
    \migs{Você prova o corolário acima (Corolário~\ref{teo:completude_mat}) de uma maneira muito semelhante que você prova o segundo caso do corolário abaixo (Corolário~\ref{cor:matvaleq}), talvez seria interessante trocar a ordem deles e adaptar a prova do Corolário~\ref{teo:completude_mat} para depender da prova do Corolário~\ref{cor:matvaleq}.}
