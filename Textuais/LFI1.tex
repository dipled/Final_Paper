...
\section{Metateoremas}
    ...
    \begin{proof}[Prova do Teorema~\ref{teo:correcao}]
        ...
        % TODO: Migs: Generalizar esse counter para poder ser utilizado em todas as provas no texto

        \begin{description}[font=\mdseries\scshape]
            \item[Caso 1.] $\phi_{1}$ seja um axioma, então basta mostrar que para todo $v \in V^{\lfium{}}$, se $v(\gamma) = 1$ para todo $\gamma \in \Gamma$, então $v(\phi_{1}) = 1$. Como $\phi_{1}$ é um axioma, basta analisar todos os casos possíveis:
            
            \begin{description}[font=\mdseries\scshape]
                
                \subcaso {Provaremos $\phi_{1} = \alpha \to (\beta \to \alpha)$.} 
                Supondo que $v(\alpha \to (\beta \to \alpha)) = 0$,
                temos $v(\alpha) = 1$ e $v(\beta \to \alpha) = 0$ por $(vImp)$. Logo, $v(\beta) = 1$ e $v(\alpha) = 0$ novamente por $(vImp)$. Isto resulta numa contradição. 
                
                Portanto $v(\alpha \to (\beta \to \alpha)) = 1$.

                \subcaso {Provaremos $\phi_{1} = (\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma ))$.} Supondo $v((\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma))) = 0$.
   
                Temos, por ($vImp$), que $v(\alpha \to (\beta \to \gamma)) = 1 \text{ e } v((\alpha \to \beta) \to (\alpha \to \gamma)) = 0$.
                Portanto, por $(vImp)$, temos que $v(\alpha) = 0$ ou $v(\beta \to \gamma) = 1$, e $v(\alpha \to \beta) = 1$ e $v(\alpha \to \gamma) = 0$. 
                
                Por $(vImp)$ segue que $v(\alpha) = 1$ e $v(\gamma) = 0$, logo, $v(\beta \to \gamma) = 1$ e, portanto, $v(\beta) = 0$ ou $v(\gamma) = 1$. 
                
                Porém, como $v(\alpha \to \beta) = 1$ e $v(\alpha) = 1$, então $v(\beta) = 1$, o que resulta numa contradição. 
                Logo $v((\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma))) = 1$.

            \end{description}

        \end{description}

        % \textit{Caso 1.} $\phi_{1}$ seja um axioma, então basta mostrar que para todo $v \in V^{\lfium{}}$, se $v(\gamma) = 1$ para todo $\gamma \in \Gamma$, então $v(\phi_{1}) = 1$. Como $\phi_{1}$ é um axioma, basta analisar todos os casos possíveis:
        ...
        \textit{Subcaso 1.5.} $\phi_{1} = (\alpha \land \beta) \to \beta$. Como no caso anterior, \textit{mutatis mutandis}.
        ...
        \textit{Subcaso 1.7.} $\phi_{1} = \beta \to (\alpha \lor \beta)$. Como no caso anterior, \textit{mutatis mutandis}.
        ...
        \textit{Subcaso 1.12.} $\phi_{1} = \neg \neg \alpha \to \alpha$. Suponha $v(\neg \neg \alpha \to \alpha) = 0$. Então, por $(vImp)$, $v(\neg \neg \alpha) = 1$ e $v(\alpha) = 0$. Por $(vCf)$ temos $v(\alpha) = 1$, o que resulta numa contradição. Portanto, $v(\neg \neg \alpha \to \alpha) = 1$.

        \textit{Subcaso 1.13.} $\phi_{1} = \alpha \to \neg \neg \alpha$. Suponha $v(\alpha \to \neg \neg \alpha) = 0$. Então, por $(vImp)$, $v(\alpha) = 1$ e $v(\neg \neg \alpha) = 0$. Por $(vCe)$ temos $v(\neg \neg \alpha) = 1$, o que resulta numa contradição. Portanto, $v(\alpha \to \neg \neg \alpha) = 1$.

        \textit{Subcaso 1.14.} $\phi_{1} = \neg \circ \alpha \to (\alpha \land \neg \alpha)$. Suponha $v(\neg \circ \alpha \to (\alpha \land \neg \alpha)) = 0$. Portanto, por $(vImp)$, $v(\neg \circ \alpha) = 1$ e $v(\alpha \land \neg \alpha) = 0$. Por $(vCi)$ temos $v(\alpha) = 1$ e $v(\neg \alpha) = 1$. Ademais, por $(vAnd)$, temos $v(\alpha) = 0$ ou $v(\neg \alpha) = 0$. Podemos concluir $v(\alpha) = 0$, já que temos $v(\alpha) = 0$ ou $v(\neg \alpha) = 0$ e também temos $v(\neg \alpha) = 1$. Isto resulta numa contradição, pois temos $v(\alpha) = 1$ e  $v(\alpha) = 0$. Portanto, $v(\neg \circ \alpha \to (\alpha \land \neg \alpha)) = 1$.

        \textit{Subcaso 1.15.} $\phi_{1} = \neg (\alpha \lor \beta) \to (\neg \alpha \land \neg \beta)$. Suponha $v(\neg (\alpha \lor \beta) \to (\neg \alpha \land \neg \beta)) = 0$. Logo, por $(vImp)$, $v(\neg (\alpha \lor \beta)) = 1$ e $v(\neg \alpha \land \neg \beta) = 0$. Por $(vDM_{\lor})$ temos $v(\neg \alpha) = 1$ e $v(\neg \beta) = 1$. Ademais, por $(vAnd)$, temos $v(\neg \alpha) = 0$ ou $v(\neg \beta) = 0$. Isto, unido ao fato de termos $v(\neg \beta) = 1$, nos permite concluir $v(\neg \alpha) = 0$, o que resulta numa contradição. Portanto, $v(\neg (\alpha \lor \beta) \to (\neg \alpha \land \neg \beta)) = 1$.

        \textit{Subcaso 1.16.} $\phi_{1} = (\neg \alpha \land \neg \beta) \to \neg (\alpha \lor \beta)$. Como no caso anterior, \textit{mutatis mutandis}.

        \textit{Subcaso 1.17.} $\phi_{1} = \neg(\alpha \land \beta) \to (\neg \alpha \lor \neg \beta)$. Suponha $v(\neg(\alpha \land \beta) \to (\neg \alpha \lor \neg \beta)) = 0$. Por $(vImp)$ temos $v(\neg(\alpha \land \beta)) = 1$ e $v(\neg \alpha \lor \neg \beta) = 0$. Então, por $(vDM_{\land})$, temos $v(\neg \alpha) = 1$ ou $v(\neg \beta) = 1$. Além disso, por $(vOr)$, temos $v(\neg \alpha) = 0$ e $v(\neg \beta) = 0$. Isto nos permite concluir $v(\neg \alpha) = 1$, pois temos $v(\neg \alpha) = 1$ ou $v(\neg \beta) = 1$ e também temos $v(\neg \beta) = 0$. Isso resulta numa contradição, pois temos $v(\neg \alpha) = 1$ e $v(\neg \alpha) = 0$. Portanto, $v(\neg(\alpha \land \beta) \to (\neg \alpha \lor \neg \beta)) = 1$.

        \textit{Subcaso 1.18.} $\phi_{1} = (\neg \alpha \lor \neg \beta) \to \neg (\alpha \land \beta)$. Como no caso anterior, \textit{mutatis mutandis}.

        \textit{Subcaso 1.19.} $\phi_{1} = \neg (\alpha \to \beta) \to(\alpha \land \neg \beta)$. Suponha $v(\neg (\alpha \to \beta) \to (\alpha \land \neg \beta)) = 0$. Por $(vImp)$ temos $v(\neg (\alpha \to \beta)) = 1$ e $v(\alpha \land \neg \beta) = 0$. Então, por $(vCIp_{\to})$, temos $v(\alpha) = 1$ e $v(\neg \beta) = 1$. Ademais, por $(vAnd)$, temos $v(\alpha) = 0$ ou $v(\neg \beta) = 0$, o que unido ao fato de termos $v(\neg \beta) = 1$, nos permite concluir $v(\alpha) = 0$. Isto resulta numa contradição, portanto $v(\neg (\alpha \to \beta) \to (\alpha \land \neg \beta)) = 1$.

        \textit{Subcaso 1.20.} $\phi_{1} = (\alpha \land \neg \beta) \to \neg(\alpha \to \beta)$. Como no caso anterior, \textit{mutatis mutandis}.

        \noindent Com isso, o Caso 1 está provado e $\Gamma \conval \phi_{1}$ segue caso $\phi_{1}$ seja um axioma.

        \textit{Caso 2.} $\phi_{1} \in \Gamma$. Logo, se $v(\gamma) = 1$ para todo $\gamma \in \Gamma$, temos $v(\phi_{1}) = 1$. Portanto, $\Gamma \conval \phi_{1}$.

         \noindent \underline{Passo} \textit{Hipótese de indução (HI): Para qualquer sequência da derivação de $\Gamma \conhil \alpha$ de tamanho $k < n$, tem-se $\Gamma \conval \alpha$}. Portanto, é preciso mostrar que $\Gamma \conval \alpha$ segue caso a sequência de derivação de $\Gamma \conhil \alpha$ tenha tamanho $n$. Ao analisar a obtenção de $\phi_{n} = \alpha$ em $\Gamma \conhil \phi_{n}$, existem três casos a se considerar:
         ...
         \textit{Caso 3.} $\phi_{n}$ é obtido por \textit{modus ponens} em duas fórmulas $\phi_{j}$ e $\phi_{k}$ com $j, k < n$. Logo, $\phi_{k} = \phi_{j} \to \phi_{n}$ (ou $\phi{j} = \phi_{k} \to \phi_{n}$, a prova para este caso é análoga). Dada nossa suposição de $\Gamma \conhil \phi_{n}$, então $\Gamma \conhil \phi_{j}$ e $\Gamma \conhil \phi_{j} \to \phi_{n}$. Pela (HI), temos $\Gamma \conval \phi_{j}$ e $\Gamma \conval \phi_{j} \to \phi_{n}$. Então, se $v(\gamma) = 1$ para todo $v(\gamma) \in \Gamma$, temos $v(\phi_{j}) = 1$ e $v(\phi_{j} \to \phi_{n}) = 1$. Por $(vImp)$ temos $v(\phi_{j}) = 0$ ou $v(\phi_{n}) = 1$. Isto, unido ao fato de termos $v(\phi_{j}) = 1$, nos permite concluir $v(\phi_{n}) = 1$. Portanto $\Gamma \conval \phi_{n}$ e a prova está finalizada.
