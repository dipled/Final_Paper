...
\section{Metateoremas}\label{sec:metateoremas}
    ...
    \helena{cometar a questão de $v$ não ser um homomorfismo e, por isso, a volta da equivalência ser mais fácil utilizando a correção em relação a semâantica matricial e a completude em relação a bivalorações}
    ...
    A ideia da prova da correção para a \lfium{} é, como de costume, provar que todo axioma do cálculo de Hilbert $\conhil$ é válido na semântica matricial $\conmat$ e mostrar que a regra de inferência \textit{modus ponens} preserva sua validade. No desenvolvimento da prova, as igualdades do tipo $h(\triangle \phi) = \triangle h(\phi)$ e $h(\phi \otimes \psi) = h(\phi) \otimes h(\psi)$, onde $\triangle, \otimes$ são um conectivos quaisquer, ficam implícitas e as justificativas do tipo ($\otimes$) dizem respeito a valoração correspondente ao conectivo $\otimes$ em $\pazocal{M}_{\lfium{}}$.
    ...
    \begin{proof}[Prova do Teorema~\ref{teo:correcao_mat}]
        ...
        \begin{provaporcasos}
            ...
            \casodeprova{} $\phi_{1}$ é um axioma. Então basta mostrar que para todo $h : \ling{} \to M$ de $\pazocal{M}_{\lfium{}}$, se $h[\Gamma] \subseteq \ummeio{}$, então $h(\phi_{1}) \in \ummeio{}$. Como $\phi_{1}$ é um axioma, basta analisar todos os casos possíveis:

            \begin{provaporsubcasos}
                
                \subcasodeprova{} $\phi_{1} = \alpha \to (\beta \to \alpha)$.

                    Supondo $h(\alpha \to (\beta \to \alpha)) = 0$, temos:

                        $h(\alpha \to (\beta \to \alpha)) = 0$

                        $\Longleftrightarrow h(\alpha) \in \ummeio{}$ e $h(\beta \to \alpha) = 0$  \hfill[$\to$]

                        $\Longleftrightarrow$ $h(\alpha) \subseteq \ummeio{}$ e $h(\beta) \subseteq \ummeio{}$ e $h(\alpha) = 0$ \hfill[$\to$]


                    Chegamos numa contradição, portanto $h(\alpha \to (\beta \to \alpha)) \in \ummeio{}$

                \subcasodeprova{} $\phi_{1} = (\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma ))$.

                    Supondo $h((\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma ))) = 0$, temos:

                    $h((\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma ))) = 0$

                    $\Leftrightarrow h(\alpha \to (\beta \to \gamma)) \in \ummeio{}$ e $h((\alpha \to \beta) \to (\alpha \to \gamma )) = 0$ \hfill[$\to$]

                    $\Leftrightarrow (h(\alpha) = 0$ ou $h(\beta \to \gamma) \subseteq \ummeio{})$ e $h(\alpha \to \beta) \subseteq \ummeio{}$ e $h(\alpha \to \gamma) = 0$ \hfill[$\to$]

                    $\Leftrightarrow (h(\alpha) = 0$ ou $h(\beta) = 0$ ou $h(\gamma) \subseteq \ummeio{})$ e ($h(\alpha) = 0$ ou $h(\beta) \subseteq \ummeio{}$) e $h(\alpha) \subseteq \ummeio{}$ e $ h(\gamma) = 0$ \hfill[$\to$]
                
                    Chegamos numa contradição, portanto $h((\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma ))) \in \ummeio{}$
            ...
            \casodeprova{} $\phi_{1} \in \Gamma$. Logo, se $v[\Gamma] \subseteq \{1\}$, temos $v(\phi_{1}) = 1$. Portanto, $\Gamma \conmat \phi_{1}$.
    ...
    \begin{proof}[Prova do Corolário~\ref{cor:lfi_forte}]
        ...
        \begin{itemize}
            \item[\textbf{(i.a)}] Vamos provar $a, \neg a \nconhil b$.
                                  Tomando a valoração $v_3$, tal que $v_3(b) = 0$, então temos $a, \neg a \nconval b$. Pela contraposta da correção (Teorema~\ref{teo:correcao_val}) temos  $a, \neg a \nconhil b$.
    ...
    \helena{KAQUI VAMOS FALAR SOBRE A SEÇÃO DO COQ}
