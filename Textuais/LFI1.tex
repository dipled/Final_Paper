...
\section{Metateoremas}\label{sec:metateoremas}
    ...
    No desenvolvimento das provas que seguem a abreviação MTD representará o metateorema da dedução. \migs{Como você colocou ``MTD'' na tabela de abreviaturas, essa linha é desnecessária.}
    ...
    \begin{proof}[Prova da Proposição~\ref{prop:tarski}]
        ...
        \end{itemize}
        \begin{adjustwidth}{1cm}{}
            \textbf{Prova do item (i)}. Vamos supor $\phi \in \Gamma$. Então \migscortar{a seguinte derivação mostra $\Gamma \conhil \phi$:}{, por definição de derivação, temos que $\Gamma \conhil \phi$.}
            % \begin{align*}
            %     1.~& \phi\tag{Premissa}
            % \end{align*}

            \noindent{}\textbf{Prova do item (ii)}. Vamos supor $\Delta \conhil \phi$ e $\Delta \subseteq \Gamma$. Então, existe uma prova (sequência de fórmulas) de $\phi$ a partir do conjunto de fórmulas $\Delta$. \migs{Como, pela suposição, temos que essa sequência está inteiramente contida em $\Gamma$ concluímos que} \cortar{a mesma sequência de fórmulas prova} $\Gamma \conhil \phi$.
            ...
            \begin{provaporcasos}
                ...
                Então, $\phi_j = \phi_k \to \phi_n$ (ou $\phi_k = \phi_j \to \phi_n$, a prova para este caso é análoga). Além disso, temos $\Delta \conhil \phi_k$ e $\Delta \conhil \phi_k \to \phi_n$. Logo, por (HI), temos $\Gamma \conhil \phi_k$ e $\Gamma \conhil \phi_k \to \phi_n$. Então, basta \cortar{derivar estas duas fórmulas e} aplicar MP para obter $\phi_n$. Portanto, $\Gamma \conhil \phi_n$.
            \end{provaporcasos}
        \end{adjustwidth}
    \end{adjustwidth}
    ...
    \begin{proof}[Prova da Proposição~\ref{prop:finit}]
        ...
        Vamos definir o conjunto $\Gamma_0$ como sendo o conjunto formado somente pelas premissas desta derivação (\migs{logo, }$\Gamma_0$ é finito). Como todo elemento de $\Gamma_0$ é uma premissa de $\Gamma$, é evidente que $\Gamma_0 \subseteq \Gamma$. Vamos provar $\Gamma_0 \conhil \phi$ por indução no tamanho $n$ da sequência de derivação.
        ...
        \begin{adjustwidth}{1cm}{}
            ...
            \begin{provaporcasos}
                ...
                Então, $\phi_j = \phi_k \to \phi_n$ (ou $\phi_k = \phi_j \to \phi_n$, a prova para este caso é análoga). Além disso, temos $\Gamma \conhil \phi_k$ e $\Gamma \conhil \phi_k \to \phi_n$. Logo, por (HI), temos $\Gamma_0 \conhil \phi_k$ e $\Gamma_0 \conhil \phi_k \to \phi_n$. Então, basta \cortar{derivar estas duas fórmulas e} aplicar MP para obter $\phi_n$. Portanto, $\Gamma_0 \conhil \phi_n$.
            \end{provaporcasos}
        \end{adjustwidth}
    ...
    \begin{proof}[Prova do Lema~\ref{lem:lindenbaum}]\setbool{@fleqn}{false}
        Seja $\lfium{} = \langle \ling, \conhil \rangle$ a lógica \lfium{} \migs{Seja a lógica LFI1 a lógica LFI1 (note também que você definiu ela no enunciado do Lema)} e seja $\Gamma \cup \{\phi\} \subseteq \ling$ um conjunto de fórmulas com $\Gamma \nconhil \phi$. Arranje \migs{Como?} as fórmulas de $\ling$ numa sequência $\pazocal{C} = \phi_1, \phi_2, \ldots, \phi_i, \ldots$ e defina $\Gamma_i$ recursivamente da seguinte forma:
        \begin{align*}
                \text{(i)}~&\Gamma_0 = \Gamma\\
                \text{(ii)}~&\Gamma_i =
                \begin{cases}
        ...
        \begin{adjustwidth}{1cm}{}
            ...
            \begin{adjustwidth}{1cm}{}
                ...
                \noindent\textbf{Hipótese de indução (HI):} Para qualquer $k < i$, temos $\Gamma_k \nconhil \phi$.
                    ...
                    Como caso particular da Hipótese de Indução, temos que $\Gamma_{i-1} \nconhil \phi$. Portanto, pela \migscortar{definição}{ construção} de $\Gamma_i$, temos $\Gamma_i \nconhil \phi$.
