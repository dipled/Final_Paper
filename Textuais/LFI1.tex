...
\section{Metateoremas}\label{sec:metateoremas}
    ...
    \begin{proof}[Prova do Lema~\ref{lem:lindenbaum}]\setbool{@fleqn}{false}
        Dada $\lfium{} = \langle \ling, \conhil \rangle$ e seja $\Gamma \cup \{\phi\} \subseteq \ling$ um conjunto de fórmulas com $\Gamma \nconhil \phi$. Arranje \migs{Como?} \helena{TAMBÉM NÃO SEI} \migs{Como não sabe mulher, foi tu quem fez a prova!?} \helena{é que eu não parei pra pensar se eu preciso dizer como arranjar as fórmulas. Tipo, existe um jeito de arranjar and that's it} as fórmulas de $\ling$ numa sequência $\pazocal{C} = \phi_1, \phi_2, \ldots, \phi_i, \ldots$ e defina $\Gamma_i$ recursivamente da seguinte forma:
        \begin{align*}
    ...
    Com a prova da completude em relação a semântica de valorações, o seguinte resultado é imediato:
