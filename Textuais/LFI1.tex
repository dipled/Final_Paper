...
\section{Metateoremas}
    ...
    \begin{teorema}[Metateorema da dedução para \lfium{}]
        \label{teo:deducao}
    ...
    \begin{lema}
        \label{lem:id}
        A derivação $\conhil \alpha \to \alpha$ é válida.
    \end{lema}
    
    \begin{proof}[Prova do Lema~\ref{lem:id}]
        A seguinte sequência de derivação demonstra o teorema:
        
        \begin{align*}
            & \text{1. } (\alpha \to ((\alpha \to \alpha) \to \alpha)) \to ((\alpha \to (\alpha \to \alpha)) \to (\alpha \to \alpha))\tag{\textbf{Ax2}}\\
            & \text{2. } \alpha \to ((\alpha \to \alpha) \to \alpha)\tag{\textbf{Ax1}}\\
            & \text{3. } \alpha \to (\alpha \to \alpha)\tag{\textbf{Ax1}}\\
            & \text{4. } (\alpha \to (\alpha \to \alpha)) \to (\alpha \to \alpha)\tag{\textbf{MP 1,2}}\\
            & \text{5. } \alpha \to \alpha\tag{\textbf{MP 4,3}}
        \end{align*}
    \end{proof}
    ...
    \begin{proof}[Prova do Teorema~\ref{teo:deducao}] A prova será dividida em duas partes:\\
        ...
        \begin{align*}
            & \text{~~~~~~1. } \; \ldots\\
            & ~~~~~~\; \; \vdots \; \; \;\ddots\\
            & \text{~~~~~~n. } \alpha \to \beta\tag{\textbf{Suposição}}\\
            & \text{n + 1. } \alpha\tag{\textbf{Premissa}}\\
            & \text{n + 2. } \beta\tag{\textbf{MP n, n + 1}}
        \end{align*}
    \end{proof}
