...
\section{Axiomatização}\label{sec:axiomatizacao}
    ...
    A teoria das provas é uma das abordagens para o estudo das relações de consequência. Nela a validade de uma inferência é atestada caso haja uma \textit{demonstração} das conclusões a partir das premissas. Uma demonstração consiste em uma sequência de passos bem definidos aplicados sobre conjuntos (ocasionalmente unitários) de fórmulas, com base nos princípios de um determinado sistema de provas. A teoria das provas é sintática\footnote{Vale notar que a separação \textit{prova {---} derivação {---} sintaxe {---} semântica {---} modelo} não é tão bem definida, algo que é explorado em~\citeshort{Prawitz2005-PRALCA-2}.} por natureza, ou seja, numa inferência $A \vdash B$, é relevante apenas a estrutura das fórmulas presentes em \textit{A} e \textit{B}, não sua interpretação ou valor-verdade. Essa estrutura é manipulada a fim de obter-se uma sequência de passos que {---} além de atestar sua validade {---} serve como argumento para tal~\cite{sep-logical-consequence}. Desta forma, pode-se definir um sistema de provas para servir como relação de consequência para uma determinada lógica. 

    No contexto da \lfium{}, existem dois sistemas de derivação estabelecidos até o momento: um cálculo de Hilbert, descrito pelos autores~\citeshort{carnielli2000formal,Carnielli_Coniglio_2016} e um sistema de \textit{Tableau}, descrito por~\citeshort{tableaulfi}. No presente trabalho, foi escolhido o cálculo de Hilbert para definir o sistema dedutivo da \lfium{}, dada a maior facilidade para desenvolver metateoremas em contraste ao sistema de \textit{Tableau}.
    ...
    \begin{definicao}[Cálculo de Hilbert para \lfium{}]\label{def:hilbert_lfi1}
        A lógica \lfium{} é definida a partir da relação de derivabilidade $\conhil{}$ sobre a linguagem $\ling{}$ através do seguinte cálculo de Hilbert:
    ...
    \begin{definicao}[Derivação em \lfium{}]\label{def:derivacao}
...
\section{Semântica}\label{sec:semantica}
    ...
    \subsection{Semântica Matricial}
        ...
        \begin{definicao}[Matriz Lógica]\label{def:matlog}
       ...
       Uma lógica que apresenta três elementos no domínio de sua matriz lógica é dita \textit{trivalorada}. Algumas lógicas trivaloradas introduzem um terceiro valor além da verdade e falsidade para representar uma informação desconhecida, como é o caso da lógica de Kleene~\cite{manyvalued}. A \lfium{}, por sua vez, introduz o valor $\meio{}$ além dos valores clássicos $0$ e $1$ para representar uma informação inconsistente. Ou seja, caso $\alpha$ tenha o valor $\meio{}$, então $\neg \alpha$ também terá o valor $\meio{}$.
        ...
        \begin{definicao}[Matriz lógica da \lfium{}]\label{def:mat}
        ...
        \begin{definicao}[Relação de consequência semântica $\conmat$]\label{def:conmat}
...
\section{Metateoremas}\label{sec:metateoremas}
        ...
        \begin{proof}[Prova da Proposição~\ref{prop:finit}]
            ...
            \begin{adjustwidth}{1cm}{}
                ...
                \begin{provaporcasos}
                    ...
                    \casodeprova{$\phi_n \in \Gamma$}. Análogo ao caso da base.
    ...
    \subsection{Correção}\label{sec:cor}
        ...
        \begin{proof}[Prova do Lema~\ref{lem:matval}]
            ...
            \begin{provaporcasos}
                ...
                \begin{provaporsubcasos}
                    \subcasodeprova{$h(\phi) = h(\alpha \to \beta) = h(\alpha) \to h(\beta) = 1$ $\Longleftrightarrow$ $v(\phi) = v(\alpha \to \beta) = 1$ e $v(\neg \phi) = v(\neg (\alpha \to \beta)) = 0$.}
                        \begin{align*}
                    ...
                    \subcasodeprova{$h(\phi) = h(\alpha \to \beta) = h(\alpha) \to h(\beta) = \meio{}$ $\Longleftrightarrow$ $v(\phi) = v(\alpha \to \beta) = 1$ e $v(\neg \phi) = v(\neg (\alpha \to \beta)) = 1$.}
                        \begin{align*}
            ...
            \begin{provaporcasos}
                ...
                \begin{provaporsubcasos}
                    \subcasodeprova{} $\phi_{1} = \alpha \to (\beta \to \alpha)$
                        \begin{center}
                            \setbool{@fleqn}{false}
                                \begin{longtable}{| C | C | C |}%chktex 44
                                    \hline%chktex 44
                    ...
                    \begin{center}
                        \setbool{@fleqn}{false}
                            ...
                            \begin{longtable}{| C | C | C | C |}%chktex 44
                                \hline%chktex 44
                    ...
                    \begin{center}
                        \setbool{@fleqn}{false}
                            ...
                            \begin{longtable}{| C | C | C | C | C |}%chktex 44
                                \hline%chktex 44
    ...
    \subsection{Completude em relação às bivalorações}\label{sec:comp}
        ...
        \begin{proof}[Prova do Lema~\ref{lem:valoracao}]
            ...
            \begin{provaporcasos}
                ...
                \casodeprova{Cláusula (\ref{val:vand})}
                ...
                \casodeprova{Cláusula (\ref{val:vor})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vimp})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vneg})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vcon})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vci})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vdne})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vdmand})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vdmor})}
                    ...
                    \casodeprova{Cláusula (\ref{val:vcip})}
        ...
        \subsection{Equivalência entre as semânticas matricial e de valorações}\label{sec:equiv}
        ...
        \begin{corolario}[Completude em relação à semântica matricial]\label{cor:completude_mat}
        ...
        \begin{proof}[Prova do Corolário~\ref{cor:completude_mat}]
