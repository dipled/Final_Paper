...
\begin{itemize}
    \item $p, q, r\ldots$ (letras minúsculas do alfabeto latim) {--} variáveis atômicas.
    \item $A, B, C, \ldots$ (letras maiúsculas do alfabeto latim) {--} conjuntos quaisquer.
    \item $\alpha, \beta, \gamma, \ldots$ (letra minúsculas do alfabeto grego) {--} fórmulas quaisquer.
    \item $\Gamma, \Delta$ {-} conjuntos de fórmulas, chamados de teorias.
...
\label{sec:paracons}
    ...
    \begin{notacao}
        Dada uma função $f : A \to B$ e um conjunto $A' \subseteq A$, define-se a notação $f[A']$ = $\{f(a) \; | \; a \in A'\}$.
    \end{notacao}
...
\label{sec:incons}
    ...
    \begin{definicao}[\lfi{} forte para lógicas proposicionais]
        \label{def:lfi_forte_prop}
