...
\label{sec:paracons}
    ...
    Uma lógica $\mathcal{L}$ será representada como uma dupla $\mathcal{L} = \langle \pazocal{L},\Vdash \rangle$, onde $\pazocal{L}$ é sua linguagem (seu conjunto de fórmulas) e $\Vdash$ é uma relação de consequência de conclusão única, definida como $\Vdash \;\subseteq \wp(\pazocal{L})\times\pazocal{L}$ \migs{, sendo $\wp(\pazocal{L})$ o conjunto das partes de $\pazocal{L}$}. Em uma consequência do tipo $\Gamma \Vdash \alpha$ (lida como ``$\alpha$ é uma consequência de $\Gamma$.'') diz-se que o conjunto $\Gamma$ é o conjunto de premissas e $\alpha$ é a conclusão. A fim de facilitar a escrita e leitura, a seguinte notação será utilizada ao longo do texto:
    ...
    \begin{notacao}
        Dada uma função $f : A \to B$ e um conjunto $A' \subseteq A$, \migscortar{define-se a notação $f[A']$ = $\{f(a) \; | \; a \in A'\}$.}{ $f[A']$ denota o conjunto $\{f(a) \; | \; a \in A'\}$.} \migs{Creio que assim fica menos esquisito}
    \end{notacao}
...
\label{sec:incons}
    ...
    Na literatura existem outras definições para as lógicas de inconsistência formal, que relaxam a condição (ii) para obter uma definição mais uniforme.~\citeshort{Carnielli_Coniglio_2016}, definem \lfis{} \textit{fracas} da seguinte forma: \migs{No parágrafo antes da definição, você falou que uma das suas referências apresenta três definições distintas, aqui você falou que na literatura há outras definições distintas. Manter esses dois parágrafos consistentes entre si.}
