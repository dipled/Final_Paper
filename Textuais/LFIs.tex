...
\begin{itemize}
    ...
    \item O \helena{operador??? n eh bem isso como q chama esse troço} $\vdash$ para representar uma relação de consequência sintática.
    \item O \helena{operador??? n eh bem isso como q chama esse troço} $\vDash$ para representar uma relação de consequência semântica.
\end{itemize}
Ademais, as definições seguirão o mesmo caminho tomado por~\citeshort{Carnielli_Coniglio_2016}, axiomatizando as noções de consistência através do cálculo de Hilbert e, posteriormente, estabelecendo uma semântica de valoração correspondente.
