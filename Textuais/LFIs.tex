...
\begin{itemize}
    ...
    \item O operador $\vDash$ para representar uma relação de consequência semântica. \migs{Aqui vc está usando o mesmo operador que no bullet abaixo}
...
\end{definicao}
Por exemplo, a assinatura proposicional para a lógica proposicional clássica pode ser definida como $\Theta_{LPC} = \{\land^{2}, \lor^{2}, \neg^{1}, \rightarrow^{2}\}$. \migs{Sugestão 1: Escreva algo tipo ``Onde o operador $\land^{2}$ representa blah blah blah...'' para cada um deles\\ Sugestão 2: Após explicar o que é cada conectivo, escreva algo tipo ``No restante do texto os símbolos para esses conectivos serão usados sem indicar sua aridade''}
...
\begin{definicao}[Lógica Tarskiana]
    ...
    \end{align}
    Uma lógica $\mathcal{L}$ é dita \textit{finitária} caso satisfaça o seguinte: \migs{Essa definição tem dois QEDs}
    \begin{align*}
