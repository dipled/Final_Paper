...
Fact $\Delta$_maximal_nontrivial : forall (f : surjection nat Formula),
...
End .
        ...
        O fato \texttt{$\Delta$\_maximal\_nontrivial} segue utilizando a definição de sobrejeção (visto que, neste caso, considera-se a função que representa organização das fórmulas numa sequência como sendo uma sobrejeção), a definição \texttt{maximal\_nontrivial} e os fatos \texttt{$\Delta$\_does\_not\_derive\_$\varphi$}, \texttt{not\_in\_$\Gamma_i$\_derives\_$\varphi$} e \texttt{lfi1\_monotonicity}.
