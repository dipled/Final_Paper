...
Record surjection (A B: Type): Type := Build_surjection {
  ...
  su_surj : function_surjective sur_f
          ...
          O teorema \texttt{strong\_lem} representa uma versão forte do princípio do terceiro excluído, provado utilizando a versão normal do terceiro excluído juntamente com o axioma \texttt{constructive\\\_indefinite\_description}, que possibilita a definição do conjunto de . 
...
(** ~($\Gamma_{i}$ $\vdash$ $\phi$) for all i *)
Fact Gamma_i_non_trivial :
forall (i : nat) (f : nat -> Formula),
  ~((Gamma_i i f) $\vdash$ $\phi$).
Proof.$\;\ldots\;$Qed.

(** $\Delta$ $\vdash$ $\phi$0 -> $\exists$n : nat, $\Gamma_{n}$ $\vdash$ $\phi$0 *)
...
(** $\psi$ $\not \in$ $\Delta$ -> $\forall$n : nat, $\psi$ $\not \in$ $\Gamma_{n}$ *)
Fact not_in_Delta_Gamma_i : forall ($\psi$ : Formula) (f : nat -> Formula),
  $\psi$ $\not \in$ (Delta f) -> forall n : nat, $\psi$ $\not \in$ (Gamma_i n f).
Proof.$\;\ldots\;$Qed.

(** $\phi_{i}$ $\not \in$ $\Gamma_{i + 1}$ -> $\Gamma_{i}$ $\cup$ {$\phi_{i}$} $\vdash$ $\phi$ *)
