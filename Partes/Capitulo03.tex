...
\section{Metateoremas}
    ...
    \begin{proof}[Prova do Teorema~\ref{teo:deducao}] A prova será dividida em duas partes:\\
        ...
        \noindent 1. Caso $\phi_{1}$ seja um axioma, a seguinte dedução mostra $\Gamma \conhil \alpha \to \phi_{1}$:
        \begin{align*}
        ...
        \noindent 2.2. Caso $\phi_{1} \in \Gamma$, então $\Gamma \conhil \alpha \to \phi_{1}$ é provado pela seguinte sequência de derivações:
        \begin{align*}
    ...
    \begin{teorema}[Correção]
        ...
        \centering
        $\Gamma \conhil \alpha \Longleftrightarrow \Gamma \conval \alpha$.
    \end{teorema}
    ...
    \begin{proof}[Prova do Teorema~\ref{teo:correcao}]
        ...
        \noindent 1. Caso $\phi_{1}$ seja um axioma, então basta mostrar que para todo $v \in V^{\lfium{}}$, se $v(\gamma) = 1$ para todo $\gamma \in \Gamma$, então $v(\phi_{1}) = 1$. Como $\phi_{1}$ é um axioma, basta analisar todos os casos possíveis:

        \noindent 1.1. Caso $\phi_{1} = \alpha \to (\beta \to \alpha)$. Suponha $v(\alpha \to (\beta \to \alpha)) = 0$. Logo $v(\alpha) = 1$ e $v(\beta \to \alpha) = 0$ por $(vImp)$. $v(\beta) = 1$ e $v(\alpha) = 0$ novamente por $(vImp)$. Isto resulta numa contradição. Portanto $v(\alpha \to (\beta \to \alpha)) = 1$.

        \noindent 1.2. Caso $\phi_{1} = (\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma ))$. Suponha $v((\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma))) = 0$. Logo (por ($vImp$)) $v(\alpha \to (\beta \to \gamma)) = 1$ e $v((\alpha \to \beta) \to (\alpha \to \gamma)) = 0$. Portanto (por $(vImp)$) $v(\alpha) = 0$ ou $v(\beta \to \gamma) = 1$. Além disso, $v(\alpha \to \beta) = 1$ e $v(\alpha \to \gamma) = 0$. Por $(vImp)$ segue que $v(\alpha) = 1$ e $v(\gamma) = 0$, logo, $v(\beta \to \gamma) = 1$ e, portanto, $v(\beta) = 0$ ou $v(\gamma) = 1$. Porém, como $v(\alpha \to \beta) = 1$ e $v(\alpha = 1)$, então $v(\beta) = 1$, o que resulta numa contradição. Portanto $v((\alpha \to (\beta \to \gamma)) \to ((\alpha \to \beta) \to (\alpha \to \gamma))) = 1$.

        \noindent 1.3. Caso $\phi_{1} = \alpha \to (\beta \to (\alpha \land \beta))$. Suponha $v(\alpha \to (\beta \to (\alpha \land \beta))) = 0$. Logo $v(\alpha) = 1$ e $v(\beta \to (\alpha \land \beta)) = 0$ por $(vImp)$. Então $v(\beta) = 1$ e $v(\alpha \land \beta) = 0$ novamente por $(vImp)$. Com isso, temos $v(\alpha) = 0$ ou $v(\beta) = 0$ por $(vAnd)$, mas isso resulta numa contradição, já que $v(\alpha) = 1$ e $v(\beta) = 1$. Portanto $v(\alpha \to (\beta \to (\alpha \land \beta))) = 1$.

        \noindent 1.4. Caso $\phi_{1} = (\alpha \land \beta) \to \alpha$. Suponha $v((\alpha \land \beta) \to \alpha) = 0$. Logo $v(\alpha \land \beta) = 1$ e $v(\alpha) = 0$ por $(vImp)$. Então, $v(\alpha) = 1$ e $v(\beta) = 1$ por $(vAnd)$, o que resulta numa contradição. Portanto $v((\alpha \land \beta) \to \alpha) = 1$.

        \noindent 1.5. Caso $\phi_{1} = (\alpha \land \beta) \to \beta$. Como no Caso 1.4, \textit{mutatis mutandis}.

        \noindent 1.6. Caso $\phi_{1} = \alpha \to (\alpha \lor \beta)$. Suponha $v(\alpha \to (\alpha \lor \beta)) = 0$. Logo $v(\alpha) = 1$ e $v(\alpha \lor \beta) = 0$ por $(vImp)$. Então temos $v(\alpha) = 0$ e $v(\beta) = 0$ por $(vOr)$, o que resulta numa contradição. Portanto $v(\alpha \to (\alpha \lor \beta)) = 1$.

        \noindent 1.7. Caso $\phi_{1} = \beta \to (\alpha \lor \beta)$. Como no Caso 1.6, \textit{mutatis mutandis}.
